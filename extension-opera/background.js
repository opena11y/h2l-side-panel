...
getOptions().then( (options) => {
  debug && console.log(`[onInstalled][options]: ${options}`);
});
...
*/
  ...
  if (request.tocSidebarMessage === "color-theme-dark") {
      debug && console.log(`[onMessage][color-theme-dark]`);
      setIcons('dark');
  }

  if (request.tocSidebarMessage === "color-theme-light") {
...
function setIcons(scheme='light') {
  ...
  if (['light', 'dark'].includes(scheme)) {
    browserAction.setIcon({
      path: {
        32: `icons/${scheme}-toc-sidebar-32.png`,
        48: `icons/${scheme}-toc-sidebar-48.png`,
        64: `icons/${scheme}-toc-sidebar-64.png`,
        128: `icons/${scheme}-toc-sidebar-128.png`
      },
    });
  }
}
